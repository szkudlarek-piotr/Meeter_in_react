...
WITH visits_count AS ( 
    ...
    for (let human of humansArr) {
        ...
        const humanPhotoDir = (fs.existsSync(path.join("photos", `${idAsString}.jpg`)) ? `/human-photo/${idAsString}.jpg`: `/human-photo/anonymous.jpg`);        
        //const cliquePhotoDir = getCliquePhoto(human.clique_id)
        const newDict = {"name": humanName, "photoDir": humanPhotoDir, "cliqueName": human.clique_name, "cliqueId": human.clique_id, "visits": human.visits_c, "meetings": human.other_interactions_c, "quote": human.quote, "id": human.id.toString()}
