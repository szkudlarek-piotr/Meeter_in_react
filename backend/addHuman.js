...
export default async function addHuman(name, surname, gender, city, cliqueId, fbLink, photoLink) {
    const reqText = "INSERT INTO `party_people` (`ID`, `name`, `surname`, `lives_where`, `gender`, `klika_id`, `fb_link`) VALUES (NULL, ?, ?, ?, ?, ?, ?);"
    const [postRequest] = await pool.query(reqText, [name, surname, city, gender, cliqueId, fbLink])
