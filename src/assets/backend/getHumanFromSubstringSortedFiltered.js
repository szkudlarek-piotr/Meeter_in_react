...
export default async function getHumanFromSubstringSortedFiltered(deliveredSubstring, mode, excludedIdsString){
    let returnedArr = []
    const excludedIdsArr = excludedIdsString ? excludedIdsString.split(',').map(Number) : []
    const excludedPlaceholders = excludedIdsArr.map(() => '?').join(',');
    console.log(excludedIdsArr)
    let queryString
    switch (mode) {
        case "visits":
            queryString = `
                ...
                ${excludedIdsArr.length ? `AND human_visits_count.humanId NOT IN (${excludedPlaceholders})` : ''}
        ...
        const [possiblePeopleArr] = await pool.query(queryString, [`%${deliveredSubstring}%`, ...excludedIdsArr])
        for (let human of possiblePeopleArr) {
            let newHumanToAdd = {"id": human.id, "name": human.full_name, "photoDir": (fs.existsSync(path.join("photos", `${human.id}.jpg`)) ? `/human-photo/${human.id}.jpg`: `/human-photo/anonymous.jpg`)}
